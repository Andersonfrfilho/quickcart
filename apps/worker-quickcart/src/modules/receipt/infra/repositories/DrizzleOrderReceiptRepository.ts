  ...
  async findOrderReceiptData(orderId: string): Promise<OrderReceiptData | undefined> {
    const rows = await db
      .select({
        ...
        deliveryFeeInCents: orders.deliveryFeeInCents,
        deliveryTierMaxKm: orders.deliveryTierMaxKm,
    ...
    return {
      ...
      deliveryTierMaxKm: row.deliveryTierMaxKm === null ? null : Number(row.deliveryTierMaxKm),
