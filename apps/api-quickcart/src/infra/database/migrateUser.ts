...
const log = logger.child('MigrateUser')

runUserMigrations({ db, migrate })
  .then(() => log.info('user_migrations_applied'))
  .then(() => closeDatabaseConnection())
  .then(() => process.exit(0))
  .catch((error) => {
    log.error('user_migrations_failed', { error: serializeError(error) })
    process.exit(1)
  })
